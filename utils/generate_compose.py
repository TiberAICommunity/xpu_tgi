...
def get_available_gpus():
    ...
    try:
        result = subprocess.run(
            ["xpu-smi", "discovery", "-j"], capture_output=True, text=True, check=True
        )
...
def generate_tgi_service(gpu_id, base_tgi_config):
    ...
    tgi_config["container_name"] = (
        "${MODEL_NAME}_gpu" + str(gpu_id) if gpu_id != -1 else "${MODEL_NAME}"
    )
...
def generate_compose_file(gpus=None):
    ...
    for gpu in gpus:
        if gpu not in available_gpus:
            raise ValueError(
                f"GPU {gpu} not available. Available GPUs: {available_gpus}"
            )
    ...
    for gpu_id in gpus:
        ...
        new_compose["services"][service_name] = generate_tgi_service(
            gpu_id, base_tgi_config
        )
...
def write_compose_file(config, filename):
    ...
    with open(filename, "w") as f:
        yaml.dump(
            config, f, Dumper=CustomDumper, sort_keys=False, default_flow_style=False
        )


def main():
    parser = argparse.ArgumentParser(
        description="Generate docker-compose file for TGI services"
    )
    parser.add_argument(
        "--gpus", type=str, help="Comma-separated GPU IDs or -1 for all GPUs"
    )
